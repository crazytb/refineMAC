...
class Agent:
    ...
    def get_model_params(self):
        """현재 에이전트의 모델 파라미터 반환."""
        return {k: v.cpu().clone() for k, v in self.pinet.state_dict().items()}

    def update_model(self, new_params):
        """집계된 글로벌 모델을 동기화."""
        self.pinet.load_state_dict(new_params)
        
def global_update(agents):
    """FedAvg 방식으로 모든 에이전트의 모델을 평균 내어 글로벌 모델을 생성."""
    global_params = agents[0].get_model_params()
    
    for k in global_params.keys():
        global_params[k] = torch.stack([agent.get_model_params()[k] for agent in agents]).mean(dim=0)
    
    # 모든 에이전트에게 글로벌 모델 동기화
    for agent in agents:
        agent.update_model(global_params)
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(output_path + "/" + "A2Cfedavg" + "_" + topo_string + "_" + timestamp)
    ...
    for n_epi in tqdm(range(MAX_EPISODES), desc="Episodes", position=0, leave=True):
        ...
        if n_epi % FEDAVG_INTERVAL == 0:
            global_update(agents)
    ...
    csv_path = f'{csv_path}/A2Cfedavg_{topo_string}_{timestamp}.csv'
    ...
    for i, agent in enumerate(agents):
        save_model(agent.pinet, f'{model_path}/A2Cfedavg_{topo_string}_agent_{i}_{timestamp}.pth')
