...
class Pinet(nn.Module):
    def __init__(self, n_observations, n_actions):
        ...
        self.attention = AttentionLayer(self.hidden_space)
        self.fc1 = nn.Linear(self.hidden_space * 2, self.hidden_space)
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(output_path + "/" + "RA2C_Attention" + "_" + topo_string + "_" + timestamp)
    ...
    for n_epi in tqdm(range(MAX_EPISODES), desc="Episodes", position=0, leave=True):
        ...
        for t in tqdm(range(MAX_STEPS), desc="  Steps", position=1, leave=False):
            ...
            for agent_id, agent in enumerate(agents):
                ...
                a = m.sample().item()
                actions.append(a)
                reward_data.append({'episode': n_epi, 'step': t, 'agent_id': agent_id, 'prob of 1': prob[0, 1].item()})
    ...
    df_pivot.to_csv(f'RA2C_Attention_{topo_string}_{timestamp}.csv', index=False)
    writer.close()
    ...
    for i, agent in enumerate(agents):
        save_model(agent.pinet, f'{model_path}/RA2C_Attention_{topo_string}_agent_{i}_{timestamp}.pth')