...
class Pinet(nn.Module):
    ...
    def pi(self, x, hidden):
        ...
        prob = F.softmax(self.actor(x), dim=2)
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(output_path + "/" + "RA2C_reversed" + "_" + topo_string + "_" + timestamp)
    ...
    df_pivot.to_csv(f'RA2C_reversed_{topo_string}_{timestamp}.csv', index=False)
    writer.close()
    ...
    for i, agent in enumerate(agents):
        save_model(agent.pinet, f'{model_path}/RA2C_reversed_{topo_string}_agent_{i}_{timestamp}.pth')
